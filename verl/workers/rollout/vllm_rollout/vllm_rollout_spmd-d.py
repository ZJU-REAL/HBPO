...
class vLLMRollout(BaseRollout):
    ...
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        self.assistant_prefill="<think> I can output the correct answer directly with minimal tokens for this question"
        self.assistant_prefill_2="<think> I should reason thouroughly to get the correct answer for this question"
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        for i in range(batch_size):
            if is_validate:
                ...
                origin_idx_2.extend(assistant_prefill_ids_2)
        ...
        if batch_size*2 != len(non_tensor_batch['raw_prompt_ids']) and batch_size != len(non_tensor_batch['raw_prompt_ids']):
        ...
        with self.update_sampling_params(**kwargs):
            ...
            if self.sampling_params.n > 1 :
                #idx_size (16,1024)
                idx = _repeat_interleave(idx, self.sampling_params.n*2)
                #print("idx",idx.size()) #idx_size (16*8*2,1024)
                attention_mask = _repeat_interleave(attention_mask, self.sampling_params.n*2)
                position_ids = _repeat_interleave(position_ids, self.sampling_params.n*2)
                batch_size = batch_size * self.sampling_params.n*2
