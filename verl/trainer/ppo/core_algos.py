...
def compute_grpo_outcome_advantage(token_level_rewards: torch.Tensor,# 
    ...
    budgets = budget.sum(dim=-1) # (bs, response_length) -> (bs,)
    

    id2score = defaultdict(list)
    id2budget = defaultdict(list)
    id2mean = {}
    id2std = {}
    id2aintra = {}
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            ...
            id2budget[index[i]].append(budgets[i]) # 
        for idx in id2score: # 
            ...
            if len(id2score[idx]) == 1:
                ...
                id2aintra[idx] = torch.tensor(0.0) 
                ...
                budget_list = torch.stack(id2budget[idx])
                scores_list = torch.stack(id2score[idx])
                
                
                budget_to_submean = {}
                unique_budgets = torch.unique(budget_list)
                
                for budget_val in unique_budgets:
                    
                    mask = (budget_list == budget_val)
                    same_budget_scores = scores_list[mask]
                    
                    budget_to_submean[budget_val.item()] = torch.mean(same_budget_scores)
                
               
                id2aintra[idx] = budget_to_submean  
        ...
        for i in range(bsz):
            ...
            if isinstance(id2aintra[index[i]], dict):
                
                current_budget = budgets[i].item()
                submean = id2aintra[index[i]][current_budget]
                PI_APPROX = 3.1416
                aintra = submean - 4 * torch.cos(torch.tensor(PI_APPROX * current_budget / 8192))
                scores[i] = scores[i] + aintra
            else:
                
                scores[i] = scores[i] + id2aintra[index[i]]
            scores[i] =scores[i] + aintra
